...
const {
  ...
  articleNotExist,
  categoryExists,
} = require("../middlewares/articlesMW");
...
app.get("/categories/:slug", categoryExists, (req, res) => {
  fs.readFile("./articles.json", (err, data) => {
    ...
    const stringified = data.toString();
    const dataJsoned = JSON.parse(stringified);
    const articles = dataJsoned.filter(
      (art) => art.category === req.params.slug
    );
...
app.delete(
  ...
  (req, res) => {
    ...
    res
      .status(201)
      .json(`The article "${req.article.title}" has been successfully deleted`);
  }
);
