...
const articleExists = (req, res, next) => {
  fs.readFile("./articles.json", (err, data) => {
    ...
    const article = dataJsoned.find(
      (art) => art.slug === req.params.slugArticle
    );
    const articleIndex = dataJsoned.findIndex(
      (art) => art.slug === req.params.slugArticle
    );
    ...
    if (article) {
      ...
      res
        .status(404)
        .json([{ msg: "Article doesn't exists", param: "global" }]);
    }
  });
};
...
const articleNotExist = (req, res, next) => {
  fs.readFile("./articles.json", (err, data) => {
    ...
    if (!article) {
      ...
      res
        .status(409)
        .json([{ msg: "Article title already exists", param: "title" }]);
    }
  });
};
...
const categoryExists = (req, res, next) => {
  fs.readFile("./categories.json", (err, data) => {
    ...
    const dataJsoned = JSON.parse(data.toString());
    const category = dataJsoned.find(
    ...
    if (category) {
      ...
      res
        .status(404)
        .json([{ msg: "Category doesn't exists", param: "category" }]);
    }
  });
};
